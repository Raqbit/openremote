...
import static org.openremote.model.attribute.Attribute.attributeFromJson;
...
public class GatewayService extends RouteBuilder implements ContainerService, AssetUpdateProcessor {
    ...
    protected void processGatewayChange(Asset gateway, PersistenceEvent<Asset> persistenceEvent) {
        ...
        switch (persistenceEvent.getCause()) {
            ...
            case UPDATE:
                ...
                // Check if disabled
                boolean isNowDisabled = gateway.getAttribute("disabled").flatMap(Attribute::getValueAsBoolean).orElse(false);
                if (isNowDisabled) {
                    connector.sendMessageToGateway(new GatewayDisconnectEvent(GatewayDisconnectEvent.Reason.DISABLED));
                }
    ...
    protected void createGatewayClient(Asset gateway) {
        ...
        try {
            ...
            gateway.getAttribute("clientId").ifPresent(assetAttribute -> assetAttribute.setValue(clientId));
            gateway.getAttribute("clientSecret").ifPresent(assetAttribute -> assetAttribute.setValue(secret));
            try {
    ...
    protected void destroyGatewayClient(Asset gateway) {
        String id = gateway.getAttribute("clientId").flatMap(Attribute::getValueAsString).orElse(null);
